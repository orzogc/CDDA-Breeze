                        ...
                        float /*difficulty*/, dealt_projectile_attack const *const proj )
{
    ...
    map &here = get_map();
    const optional_vpart_position veh_part = here.veh_at( pos() );
    const bool in_skater_vehicle = in_vehicle &&
                                   veh_part.part_with_feature( "SEAT_REQUIRES_BALANCE", false );
...
int Character::free_upgrade_points() const
{
    int lvl = 0;
...
void Character::upgrade_stat_prompt(const character_stat& stat)
{
    ...
    if (get_player_character().query_yn("你确定要提升 %s 吗？当前可用点数 %d 点。", stat_string,
        free_points)) {
        switch (stat) {
        case character_stat::STRENGTH:
            str_max++;
