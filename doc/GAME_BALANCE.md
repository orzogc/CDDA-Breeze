...
For both critical and non-critical hits, average damage is calculated based on the weapon's stats and the user's skill.  Monster armor absorbs the damage, and then the damage is multiplied by the number of hits: either critical hits for the critical hit case, or total hits - critical hits for the non critical hit case.  Rapid strike attacks now keep full per-hit damage and use 75% of the normal move cost.  This keeps their unarmored DPS bonus close to the previous implementation while avoiding the extra armor-threshold penalty caused by reducing per-hit damage.

Number of moves is calculated from the weapon's normal attack speed for misses and ordinary hits.  When rapid strike is available, the rapid half of successful attacks is modeled at 75% of normal move cost while retaining the same damage calculation.
