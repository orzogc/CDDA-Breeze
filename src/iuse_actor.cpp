...
std::optional<int> link_up_actor::use(Character& p, item& it, bool t, const tripoint&) const
{
    ...
    if (choice == 0 || choice == 1) {
        ...
        const std::optional<tripoint> pnt_ = choose_adjacent_highlight("将电缆连接到哪里？",
        ...
        if (cable->link->has_no_links()) {
            ...
            point vcoords = cable->link->t_mount;
            vehicle_part prev_part(vpid, vpid.str(), vcoords,vpid_found ? item(it) : item("hd_tow_cable"));
            ...
            vcoords = t_vp->mount();
            vehicle_part target_part(vpid, vpid.str(), vcoords, vpid_found ? item(it) : item("hd_tow_cable"));
