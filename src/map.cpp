...
vehicle *map::move_vehicle( vehicle &veh, const tripoint &dp, const tileray &facing )
{
    ...
    veh.precalc_mounts( 1, vertical && veh.is_falling ? veh.face.dir() :
                        ( veh.skidding ? veh.turn_dir : facing.dir() ), veh.pivot_point() );
...
{
    const tripoint viewer_p = viewer.pos();
    float eye_level = 1.0f;
    switch( viewer.get_size() ) {
    ...
    if( const Character *viewer_ch = viewer.as_character() ) {
                         ...
                         const float eye_level ) const
{
    ...
    for( const tripoint &p : line_to( tripoint( low_p.xy(), high_p.z ), high_p ) ) {
    ...
    constexpr float zlevel_to_grid_ratio = 2.0f;
    ...
    float result = ( covered_z - target_p.z * zlevel_to_grid_ratio ) * 100.0f;

    const furn_id target_furn = furn( target_p );
    if( target_furn ) {
        result -= target_furn->coverage;
    } else if( move_cost( target_p ) > 2 &&
               !has_flag_ter( ter_furn_flag::TFLAG_FLAT, target_p ) ) {
        result -= ter( target_p )->coverage;
    }
    return std::max( static_cast<int>( result ), 0 );
}
